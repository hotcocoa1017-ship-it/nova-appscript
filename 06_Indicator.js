...
function updateRoomOperation(token, payload) { // (객실 청소배정·상태변경·실행시간 측정)
  return measureResponse_('updateRoomOperation', () => {
    ...
    // 청소초기화도 PostgreSQL을 원본으로 사용합니다. // CLEANING_RESET_SERVER_DB_FIRST_V1
    ...
    // 퇴실 외 일반 객실상태 변경도 PostgreSQL을 원본으로 사용합니다. // ROOM_STATUS_SERVER_DB_FIRST_V1
...
function changeIndicatorRoomStatusDbFirst_(token, user, payload, context) { // (일반 객실상태 PostgreSQL 원본 경로) // ROOM_STATUS_SERVER_DB_FIRST_V1
  const safe = payload || {};
  const info = context || {};
  const businessDate = info.businessDate;
  const site = String(info.site || '').trim();
...
function clearIndicatorRoomAssignmentDbFirst_(token, user, payload, context) { // (배정 초기화 PostgreSQL 원본 경로) // CLEAR_ASSIGNMENT_SERVER_DB_FIRST_V1
  const safe = payload || {};
  const info = context || {};
  const businessDate = info.businessDate;
  const site = String(info.site || '').trim();
...
function assignIndicatorRoommaidDbFirst_(token, user, payload, context) { // (룸메이드 배정 PostgreSQL 원본 경로) // ROOMMAID_ASSIGN_SERVER_DB_FIRST_V1
  const safe = payload || {};
  const info = context || {};
  const businessDate = info.businessDate;
  const site = String(info.site || '').trim();
...
function startIndicatorRoomCleaningDbFirst_(token, user, payload, context) { // (청소시작 PostgreSQL 원본 경로) // CLEANING_START_SERVER_DB_FIRST_V1
  const safe = payload || {};
  const info = context || {};
  const businessDate = info.businessDate;
  const site = String(info.site || '').trim();
...
function completeIndicatorRoomCleaningDbFirst_(token, user, payload, context) { // (청소완료 PostgreSQL 원본 경로) // CLEANING_COMPLETE_SERVER_DB_FIRST_V1
  const safe = payload || {};
  const info = context || {};
  const businessDate = info.businessDate;
  const site = String(info.site || '').trim();
...
function resetIndicatorRoomCleaningDbFirst_(token, user, payload, context) { // (청소완료 실적초기화 PostgreSQL 원본 경로) // CLEANING_RESET_SERVER_DB_FIRST_V1
  const safe = payload || {};
  const info = context || {};
  const businessDate = info.businessDate;
  const site = String(info.site || '').trim();
