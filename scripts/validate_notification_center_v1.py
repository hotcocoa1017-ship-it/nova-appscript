...
require(center, "nova:user:${a.employee}:notifications", 'per-user notification topic')
require(center, "claims(a.token)?.employee_no", 'employee number claim binding')
require(center, "novaNotificationSound", 'notification sound preference')
require(center, 'function sound()', 'notification sound playback')
require(center, 'async function readAll()', 'mark-all-read action')
require(center, 'function route(x)', 'notification click routing')
require(center, "Authorization':`Bearer ${U.auth.token}`", 'user JWT used for Data API')
