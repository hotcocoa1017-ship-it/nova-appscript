...
-- Purpose:
-- 1) validate every room version from the preview snapshot, not only max(version)
-- 2) atomically upsert/delete the complete site/day room set
-- 3) preserve all nova_rooms_current operational and previous-cycle fields
-- 4) retain request-id idempotence and ADMIN/ORDER site authorization
...
begin
  ...
  -- Reject blank room numbers and duplicate room numbers before taking write locks.
  if exists (
    select 1
  ...
  -- Every uploaded room must carry an expected-version token.
  if exists (
    select 1
    from jsonb_array_elements(p_rooms) r
  ...
  -- All expected-version values must be unsigned integers.
  if exists (
  ...
  -- Serialize full-room uploads for the same business date/site.
  perform pg_advisory_xact_lock(hashtextextended('NOVA_ROOM_UPLOAD_V4|'||v_date::text||'|'||v_site,0));

  -- Lock every current row in the scope so no room action can change it after validation.
  perform 1
  ...
  -- Any DB room not represented by the preview snapshot means the scope changed after preview.
  if exists (
    select 1
    from public.nova_rooms_current c
    where c.business_date=v_date and c.site=v_site
  ...
  -- Every existing DB room must match its exact preview-time version.
  if exists (
    select 1
    from public.nova_rooms_current c
    where c.business_date=v_date and c.site=v_site
  ...
  -- An expected key that no longer exists is only valid for a genuinely new uploaded room with expectedVersion=0.
  if exists (
    select 1
  ...
  -- A payload room that did not exist at preview time must explicitly carry expectedVersion=0.
  if exists (
    select 1
    from jsonb_array_elements(p_rooms) r
    where not exists (
  ...
  -- DB owns the committed upload version. Preserve a higher caller data-version when supplied.
  ...
  -- Delete rooms absent from the committed upload only after all expected-version checks pass.
