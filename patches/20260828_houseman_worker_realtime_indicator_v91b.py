...
# HOUSEMAN GET endpoint: ADMIN/ORDER can query all orders for date/site;
# HOUSEMAN remains restricted by SQL to own assigned/processed/shared-candidate orders.
old_get = r'''      if (String(user.role || '').toUpperCase() !== 'HOUSEMAN') {
...
'''
new_get = r'''      const role = String(user.role || '').toUpperCase();
...
'''
if old_get not in cloud:
    raise SystemExit('houseman GET role/query target not found')
cloud = cloud.replace(old_get, new_get, 1)
 ...
 * DB 상태를 먼저 확정하고 기존 Sheet 업무이력/감사는 비동기 미러한다.
 */
app.post(
  ...
  async (req, res, next) => {
    ...
    try {
      ...
      if (['ACCEPT', 'ACCEPT_START'].includes(action)) {
        ...
        if (status !== required) {
          throw httpError(409, 'HOUSEMAN_ORDER_STATE_CONFLICT', `현재 ${status || '-'} 상태에서는 ${action} 처리를 할 수 없습니다.`);
        }
      }

      const acceptAction = ['ACCEPT', 'ACCEPT_START'].includes(action);
      const startAction = action === 'ACCEPT_START' || action === 'START';
      const finishAction = action === 'COMPLETE' || action === 'UNABLE';
      const targetStatus = action === 'ACCEPT' ? 'ACCEPTED'
        : (startAction ? 'PROCESSING' : (action === 'COMPLETE' ? 'COMPLETED' : 'UNABLE'));
      const released = acceptAction
      ...
      const updated = await db.query(
        `update public.nova_houseman_orders
          ...
          where order_id=$1
          returning *`,
        [orderId, targetStatus, acceptAction, employeeNo, String(user.name || employeeNo), released, startAction, finishAction, reason]
      );
...
if "'/v1/houseman-orders/:orderId/action'" not in cloud:
    if cancel_anchor not in cloud:
        raise SystemExit('houseman cancel insertion anchor not found')
    cloud = cloud.replace(cancel_anchor, worker_route + cancel_anchor, 1)
...
helpers = r'''  const novaRealtimeHousemanMirrorChains_ = new Map();
  ...
  async function novaRealtimeHousemanWorkerAction_(payload) { // (하우스맨 접수/처리 Cloud Run 전용)
    ...
    mapped.__dbVersion = Number(result.orderVersion || result.order?.version || mapped.version || 0);
    ...
    return { ok: true, realtime: true, order: mapped, version: Number(safe.expectedVersion || 0), timing: result.timing || {}, requestId };
  }

  function queueRealtimeHousemanWorkerMirror_(payload) { // (동일 오더 Sheet 미러 순서 보장)
    const safe = Object.assign({}, payload || {});
    const orderId = String(safe.orderId || '').trim();
    if (!orderId) return;
    const previous = novaRealtimeHousemanMirrorChains_.get(orderId) || Promise.resolve();
    const next = previous.catch(() => null).then(() => mirrorRealtimeHousemanWorkerActionToSheet_(safe));
    novaRealtimeHousemanMirrorChains_.set(orderId, next);
    void next.finally(() => {
      if (novaRealtimeHousemanMirrorChains_.get(orderId) === next) novaRealtimeHousemanMirrorChains_.delete(orderId);
    });
  }

  async function mirrorRealtimeHousemanWorkerActionToSheet_(payload) { // (업무이력/감사 비동기 보완)
...
if 'async function novaRealtimeHousemanWorkerAction_(' not in client:
    if helper_anchor not in client:
        raise SystemExit('client helper anchor not found')
    client = client.replace(helper_anchor, helpers + helper_anchor, 1)

old_route = """    delete legacySafe.sheetExpectedVersion;
...
"""
new_route = """    delete legacySafe.sheetExpectedVersion;
    ...
    const realtimeAction = ['CLEANING_START', 'CLEANING_COMPLETE', 'CLEANING_RESET', 'ASSIGN_ROOMMAID', 'QM_ASSIGN', 'QM_START', 'QM_COMPLETE', 'QM_REWORK', 'UPDATE_ROOM_OPERATION_STATUS', 'UPDATE_OPERATION_FLAGS', 'CLEAR_ASSIGNMENT'].includes(mappedAction)
"""
if old_route not in client:
    raise SystemExit('generic realtime route anchor not found')
client = client.replace(old_route, new_route, 1)

old_broadcast = ".on('broadcast', { event: 'UPDATE' }, payload => novaRealtimeHandleBroadcast_(payload))\n        .on('broadcast', { event: 'INSERT' }, payload => {"
new_broadcast = ".on('broadcast', { event: 'UPDATE' }, payload => {\n          novaRealtimeHandleBroadcast_(payload);\n          novaRealtimeHandleHousemanOrderBroadcast_(payload);\n        })\n        .on('broadcast', { event: 'INSERT' }, payload => {"
if old_broadcast not in client:
    raise SystemExit('broadcast UPDATE anchor not found')
client = client.replace(old_broadcast, new_broadcast, 1)

fallback_anchor = '  async function novaRealtimeHydrateIndicatorFallback_() {'
admin_helper = r'''  async function novaRealtimeHydrateIndicatorHousemanOrders_() { // (ADMIN/ORDER 하우스맨 DB 상태 직접 보정)
    ...
    const lists = await Promise.all(sites.map(site => novaRealtimeLoadHousemanOrders_(businessDate, site)
    ...
    lists.flat().forEach(raw => {
      ...
      const beforeSig = JSON.stringify([previous.statusCode, previous.assignedEmployeeNo, previous.processorEmployeeNo, previous.acceptedAt, previous.startedAt, previous.completedAt, previous.unableReason, previous.updatedAt]);
      const afterSig = JSON.stringify([next.statusCode, next.assignedEmployeeNo, next.processorEmployeeNo, next.acceptedAt, next.startedAt, next.completedAt, next.unableReason, next.updatedAt]);
...
old_call = """      if (changedCount) {
        renderIndicatorSummary();
        setSyncStatus(`DB 변경확인 · ${changedCount}실 반영`);
      }
    } finally {
"""
new_call = """      const housemanChanged = await novaRealtimeHydrateIndicatorHousemanOrders_();
      if (changedCount) {
        renderIndicatorSummary();
...
print('HOUSEMAN_WORKER_REALTIME_INDICATOR_V91B_OK')
