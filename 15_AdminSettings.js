...
function processAutomaticDailyClose(options) { // (설정시각 기준 미마감 사업장 자동 저장)
  const safe = options || {};
  ...
  try {
    ...
    return { ok: true, businessDate, closeTime, sites: results, message: `${businessDate} ${results.length}개 사업장을 자동 마감했습니다.` };
  } catch (error) {
